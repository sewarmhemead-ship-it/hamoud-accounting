...
#define NodeZip "node-v20.20.2-win-x64.zip"
...
english.FinishedLabel=تم تثبيت حمود — محاسبة تخليص بنجاح.%n%nاضغط «إنهاء» ثم شغّل البرنامج من قائمة ابدأ أو الاختصار على سطح المكتب.%n%nلا حاجة لتثبيت Node.js يدوياً — يُستخرج تلقائياً من الحزمة.

[Tasks]
Name: "desktopicon"; Description: "إنشاء اختصار على سطح المكتب (SewarTech)"; GroupDescription: "اختصارات:"; Flags: unchecked

[Files]
Source: "{#PackageDir}\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs; Excludes: "backend\data\hamoud.db-wal;backend\data\hamoud.db-shm;node-runtime\*;node-extract-tmp\*"
Source: "{#NodeZip}"; DestDir: "{app}"; Flags: ignoreversion
Source: "extract-node-runtime.ps1"; DestDir: "{app}"; Flags: ignoreversion
...
[Run]
Filename: "powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -File ""{app}\extract-node-runtime.ps1"" -AppRoot ""{app}"""; StatusMsg: "جاري تجهيز Node.js 20 المحمول (SewarTech)..."; Flags: waituntilterminated runhidden
...
begin
  ...
  if not FileExists(ExpandConstant('{#NodeZip}')) then
  begin
    Msg := 'ملف Node.js المضمّن غير موجود.' + #13#10 +
      'شغّل: installer\build-installer.ps1 لتحميل {#NodeZip}';
    MsgBox(Msg, mbError, MB_OK);
    Result := False;
    Exit;
  end;
